...
export class ReportesService {
  ...
  public async actualizar(objReporte: Reportes, codigo: number): Promise<any> {
    try {
      if (await this.verificarReporte(objReporte.descripcion)) {
        return new HttpException(
          'El reporte ya existe ya existe',
          HttpStatus.BAD_REQUEST,
        );
      } else {
        const objActualizado = this.reportesRepository.update(
          { idReporte: codigo },
          objReporte,
        );
        return new HttpException(
...
// Traer todos los reportes con relaciones y ordenados por fechaReporte de forma descendente.
  public async consultarTodoConRelaciones(): Promise<any> {
    try {
        ...
        throw new HttpException('Fallo al consultar reportes con relaciones', HttpStatus.BAD_REQUEST);
    }
 }
