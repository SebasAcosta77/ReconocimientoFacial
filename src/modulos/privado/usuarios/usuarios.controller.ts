...
export class UsuariosController {
  ...
  public async registrarUsuarios(@Body() objUsuario: Usuarios) {
    try {
      ...
      // Retornar solo los campos básicos para evitar problemas de relaciones no cargadas
      return {
        mensaje: 'Usuario registrado correctamente',
        data: {
          codUsuario: resultado.codUsuario,
          nombrsUsuario: resultado.nombrsUsuario,
          apellidosUsuario: resultado.apellidosUsuario,
  ...
  public async consultarUnUsuario(@Param('codUsuario') codUsuario: string) {
    ...
    try {
      ...
      if (!usuarios.length) {
        throw new HttpException(
          `No se encontró usuario con codUsuario ${codUsuario}`,
      ...
      throw new HttpException(
        `Error al consultar usuario ${codUsuario}: ${error.message}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  public async actualizarUsuario(
    ...
    try {
      ...
      throw new HttpException(
        `Error al actualizar usuario ${codUsuario}: ${error.message}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  public async eliminarUsuario(@Param('codUsuario') codUsuario: string) {
    ...
    try {
      ...
      throw new HttpException(
        `Error al eliminar usuario ${codUsuario}: ${error.message}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  @Get('/detalle/:codUsuario')
  public async obtenerDetalleUsuario(@Param('codUsuario') codUsuario: string) {
    if (!codUsuario || codUsuario.trim() === '') {
      throw new HttpException(
        'Código de usuario no válido',
        HttpStatus.BAD_REQUEST,
    ...
    try {
      const detalle =
        await this.usuarioService.obtenerDetallesUsuario(codUsuario);
      return {
