...
export class PythonService {
  ...
  public async iniciarReconocimiento(codEvento: number): Promise<any> {
    try {
      const response = await axios.post(`${this.pythonApiUrl}/api/start`, { cod_evento: codEvento });
      return { mensaje: 'Reconocimiento iniciado correctamente', data: response.data };
    } catch (error) {
  ...
  public async detenerReconocimiento(): Promise<any> {
    try {
      ...
      return { mensaje: 'Reconocimiento detenido correctamente', data: response.data };
    } catch (error) {
