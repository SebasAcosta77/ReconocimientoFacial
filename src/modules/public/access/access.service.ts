...
export class AccessService {
  ...
  public async sesion(objAcceso: Acceso): Promise<any> {
    const usuarioExiste = await this.accesoRepository.findBy({
    ...
    if (usuarioExiste.length != 0) {
      ...
      if (compareSync(objAcceso.claveAcceso, claveAcceso)) {
        try {
          let datosSesion = await this.accesoRepository.query(
            ACCESO_SQL.DATOS_SESION,
            [usuarioExiste[0].codUsuario],
          );

          let tokenSistema = GenerarToken.procesarRespuesta(datosSesion[0]);

          if (tokenSistema != '') {
            return { tokenApp: tokenSistema }; // ✅ Correcto
          } else {
            throw new HttpException(
              'fallo al generar la autenticacion',
              HttpStatus.CONFLICT,
            );
          }
        } catch (miError) {
          throw new HttpException(
            'fallo al consultar la informacion',
