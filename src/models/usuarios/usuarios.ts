...
export class Usuarios {
    @PrimaryColumn({type: "varchar", length:12, name: "documento", nullable: false})
    ...
    @Column({type: "date",  name: "fecha_nacimiento", default: new Date(Date.now())})
    public fechaNacimientoUsuario: Date;

    @Column({type:"varchar", length:150, name: "correo_electronico", nullable: false})
    public correoUsuario: string;

    @Column({type:"varchar", length:20, name: "telefono", nullable: false})
    public telefonoUsuario: string;

    @Column({type: "integer",  name: "cod_rol", nullable:true})
    ...
    @OneToMany(() => Fotografias, (objImagen: Fotografias) => objImagen.codUsuarioI, { //muchos a uno municipio - sitio
        onDelete: "RESTRICT",
        onUpdate: "CASCADE"
    })
    ...
    @OneToMany(() => Registroasistencia, (objRegistro: Registroasistencia) => objRegistro.codReporteRegis, { //muchos a uno municipio - sitio
        onDelete: "RESTRICT",
        onUpdate: "CASCADE"
    })
    ...
    @OneToMany(() => Reportes, (objReporte: Reportes) => objReporte.codUsuarioR, { //muchos a uno municipio - sitio
        onDelete: "RESTRICT",
        onUpdate: "CASCADE"
    })
    ...
    constructor(codUsuario: string, nombrsUsuario: string, apellidosUsuario: string, fechaNacimientoUsuario: Date, correoUsuario: string, telefonoUsuario: string){
